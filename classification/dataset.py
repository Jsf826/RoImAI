...
class AirDataset(data.Dataset):
    def __init__(self, image_dir, list_path, input_transform=None, re_level='class', proportion=1.0):
        ...
        self.map = {'A300B4': 1, 'A310': 2, 'A318': 3, 'A319': 4, 'A320': 5, 'A321': 6, 'A330-200': 7, 'A330-300': 8,
                    'A340-200': 9, 'A340-300': 10, 'A340-500': 11, 'A340-600': 12, 'A380': 13, 'An-12': 14,
                    'ATR-42': 15, 'ATR-72': 16, 'BAE 146-200': 17, 'BAE 146-300': 18, 'BAE-125': 19,
                    'Beechcraft 1900': 20, '707-320': 21, 'Boeing 717': 22, '727-200': 23, '737-200': 24, '737-300': 25,
                    '737-400': 26, '737-500': 27, '737-600': 28, '737-700': 29, '737-800': 30, '737-900': 31,
                    '747-100': 32, '747-200': 33, '747-300': 34, '747-400': 35, '757-200': 36, '757-300': 37,
                    '767-200': 38, '767-300': 39, '767-400': 40, '777-200': 41, '777-300': 42, 'C-130': 43, 'C-47': 44,
                    'Cessna 172': 45, 'Cessna 208': 46, 'Cessna 525': 47, 'Cessna 560': 48, 'Challenger 600': 49,
                    'CRJ-200': 50, 'CRJ-700': 51, 'CRJ-900': 52, 'DHC-8-100': 53, 'DHC-8-300': 54, 'DC-10': 55,
                    'DC-3': 56, 'DC-6': 57, 'DC-8': 58, 'DC-9-30': 59, 'DH-82': 60, 'DHC-1': 61, 'DHC-6': 62,
                    'Dornier 328': 63, 'DR-400': 64, 'EMB-120': 65, 'E-170': 66, 'E-190': 67, 'E-195': 68,
                    'ERJ 135': 69, 'ERJ 145': 70, 'Embraer Legacy 600': 71, 'Eurofighter Typhoon': 72, 'F/A-18': 73,
                    'F-16A/B': 74, 'Falcon 2000': 75, 'Falcon 900': 76, 'Fokker 100': 77, 'Fokker 50': 78,
                    'Fokker 70': 79, 'Global Express': 80, 'Gulfstream IV': 81, 'Gulfstream V': 82, 'Hawk T1': 83,
                    'Il-76': 84, 'Model B200': 85, 'L-1011': 86, 'MD-11': 87, 'MD-80': 88, 'MD-87': 89, 'MD-90': 90,
                    'Metroliner': 91, 'PA-28': 92, 'Saab 2000': 93, 'Saab 340': 94, 'Spitfire': 95, 'SR-20': 96,
                    'Tornado': 97, 'Tu-134': 98, 'Tu-154': 99, 'Yak-42': 100}
...
class SSMG_Dataset(data.Dataset):
    def __init__(self, image_dir: str, transforms: A.Compose):
        super(SSMG_Dataset, self).__init__()
        self.image_dir = image_dir
        self.transforms = transforms
        self.sample_list = self._get_sample(self.image_dir)
        random.shuffle(self.sample_list)
        self.sample_list = self.sample_list
        # self.cls2index = cls2index
        # self.index2cls = {value: key for key, value in cls2index.items()}
        # self.trees = [
        #     [1, 1],
        #     [2, 2],
        #     [3, 3],
        ...
        self.trees_family_to_species = [
            [0, 0, 3, 4],  # 一级类别 A 包含二级类别 A1, A2
            [1, 1, 5, 6],  # 一级类别 B 包含二级类别 B1, B2
            [2, 2, 7, 8, 9, 10, 11],  # 一级类别 C 包含二级类别 C1, C2, C3, C4, C5
        ]
        # self.trees_order_to_family = [
        #     [1, 1, 4, 5],  # 一级类别 A 包含二级类别 A1, A2
        #     [2, 2, 6, 7],  # 一级类别 B 包含二级类别 B1, B2
        #     [3, 3, 8, 9, 10, 11, 12],  # 一级类别 C 包含二级类别 C1, C2, C3, C4, C5
        # ]

        # self.map = {
        #     "石英类": 1, "长石": 2, "岩屑": 3, "石英": 4, "燧石": 5, "斜长石": 6, "钾长石": 7, "沉积岩岩屑":8, "火成岩岩屑": 9,
        #     "变质岩岩屑": 10, "火山碎屑岩岩屑": 11, "其他矿物碎屑": 12
        # }
        self.map = {
            "石英类": 0, "长石": 1, "岩屑": 2, "石英": 3, "燧石": 4, "斜长石": 5, "钾长石": 6, "沉积岩岩屑": 7,
            "火成岩岩屑": 8, "变质岩岩屑": 9, "火山碎屑岩岩屑": 10, "其他矿物碎屑": 11
        }

    def _get_sample(self, image_dir):
        sample_list = []
        for first_cls_name in os.listdir(image_dir):
            for second_cls_name in os.listdir(os.path.join(image_dir, first_cls_name)):
                for fname in os.listdir(os.path.join(image_dir, first_cls_name, second_cls_name)):
                    if fname.endswith('-.jpg'):
                        sample_list.append((fname, first_cls_name, second_cls_name))
        return sample_list
    ...
    def __getitem__(self, index):
        image_name, first_cls_name, second_cls_name = self.sample_list[index]
        image_path = os.path.join(self.image_dir, first_cls_name, second_cls_name, image_name)
        input_image = cv2.imread(image_path)
        # cv2.imwrite("sad.jpg", input_image)
        if self.transforms:
            input_image = self.transforms(image=input_image)["image"]
        target = int(second_cls_name)

        return input_image, target


class Multi_Image_SSMG_Dataset(data.Dataset):
    def __init__(self, image_dir: str, transforms: A.Compose):
        super(Multi_Image_SSMG_Dataset, self).__init__()
        self.image_dir = image_dir
        self.transforms = transforms
        self.sample_list = self._get_sample(self.image_dir)
        random.shuffle(self.sample_list)
    ...
    def _get_sample(self, image_dir):
        ...
        for first_cls_name in os.listdir(image_dir):
            for second_cls_name in os.listdir(os.path.join(image_dir, first_cls_name)):
                for img_name1 in os.listdir(os.path.join(image_dir, first_cls_name, second_cls_name)):
                    if img_name1.endswith('-.jpg'):
                        img_name2 = img_name1.replace('-.jpg', '+.jpg')
                        sample_list.append((img_name1, img_name2, first_cls_name, second_cls_name))
        return sample_list

        # sample_list = []
        # for cls_name in os.listdir(image_dir):
        #     for img_name1 in os.listdir(os.path.join(image_dir, cls_name)):
        #         if img_name1.endswith('-.jpg'):
        #             img_name2 = img_name1.replace('-.jpg', '+.jpg')
        #             sample_list.append((img_name1, img_name2, cls_name))
        # return sample_list
    ...
    def __getitem__(self, index):
        # 返回两张图片
        image_name1, image_name2, first_cls_name, second_cls_name = self.sample_list[index]
        image_path1 = os.path.join(self.image_dir, first_cls_name, second_cls_name, image_name1)
        image_path2 = os.path.join(self.image_dir, first_cls_name, second_cls_name, image_name2)
        input_image1 = cv2.imread(image_path1)
        input_image2 = cv2.imread(image_path2)
        if self.transforms:
            input_aug = self.transforms(image=input_image1, image2=input_image2)
            input_image1 = input_aug["image"]
            input_image2 = input_aug["image2"]
        target = int(second_cls_name)

        return input_image1, input_image2, target
