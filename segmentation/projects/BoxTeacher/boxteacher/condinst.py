...
from .image_fusion_net import ImageFusionNet
        ...
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                if self.pseudo_mask_affinity_thresh == -1:
                    ...
                    mask_affinity_weights = (
                                                    mask_affinity >= self.pseudo_mask_affinity_thresh).float() * box_gt_bitmasks.float()
...
class CondInst(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # 定义多图像融合层
        self.pre_image_fusion_layer = ImageFusionNet()
        # self.pre_image_fusion_layer2 = ImageFusionFCN()
        # self.post_image_fusion_layer = ImageFusionFCN()
        ...
        # rock_seg configs
        self.is_fusion = cfg.ROCK_SEG.IS_FUSION
        self.fusion_mode = cfg.ROCK_SEG.FUSION_MODE
    ...
    def forward_teacher(self, batched_inputs):
        ...
        if self.is_fusion:
            if self.fusion_mode == "pre":
                return self._pre_fusion_forward_teacher(batched_inputs)
            elif self.fusion_mode == "post":
                return self._post_fusion_forward_teacher(batched_inputs)
            else:
                raise ArgumentTypeError("fusion_mode must be 'pre' or 'post'")
        else:
            return self._sin_image_forward_teacher(batched_inputs)

    @torch.no_grad()
    def _sin_image_forward_teacher(self, batched_inputs):
        ...
        for im_id, (input_per_image, proposal, image_size) in enumerate(
                zip(batched_inputs, proposals, image_sizes)):
    ...
    @torch.no_grad()
    def _pre_fusion_forward_teacher(self, batched_inputs):
        original_images1 = [x["image1"].to(self.device) for x in batched_inputs]
        original_images2 = [x["image2"].to(self.device) for x in batched_inputs]
        images_norm1 = [self.normalizer(x) for x in original_images1]
        images_norm2 = [self.normalizer(x) for x in original_images2]
        # if self.using_resize:
        # resize images to (maxsize, minsize)
        images_norm1, image_sizes = resize_images(images_norm1, self.resize_size)
        images_norm2, image_sizes = resize_images(images_norm2, self.resize_size)

        images_norm1 = ImageList.from_tensors(images_norm1, self.backbone.size_divisibility)
        images_norm2 = ImageList.from_tensors(images_norm2, self.backbone.size_divisibility)

        # 融合多图像特征
        fusion_tensor = self.pre_image_fusion_layer(images_norm1.tensor, images_norm2.tensor)

        # backbone进行特征提取
        features = self.backbone(fusion_tensor)

        gt_instances = None
        mask_feats, _ = self.mask_branch(features, gt_instances)

        fusion_image_list = [f_t for f_t in fusion_tensor]
        fusion_image_list = ImageList.from_tensors(fusion_image_list, self.backbone.size_divisibility)

        proposals, _ = self.proposal_generator.forward_test(
            fusion_image_list, features, gt_instances, self.controller)

        padded_im_h, padded_im_w = images_norm1.tensor.size()[-2:]
        processed_results = []
        # if not self.using_resize:
        #     image_sizes = images_norm.image_sizes
    ...
    @torch.no_grad()
    def _post_fusion_forward_teacher(self, batched_inputs):
        pass
        # original_images1 = [x["image"].to(self.device) for x in batched_inputs]
        # original_images2 = [x["image2"].to(self.device) for x in batched_inputs]
        # images_norm1 = [self.normalizer(x) for x in original_images1]
        # images_norm2 = [self.normalizer(x) for x in original_images2]
        # # if self.using_resize:
        ...
        #
        # # 融合多图像特征
        # # fusion_tensor = self.image_fusion_layer(images_norm1.tensor, images_norm2.tensor)
        #
        # # backbone进行特征提取
        # # features = self.backbone(images_norm.tensor)
        # features1 = self.backbone(images_norm1.tensor)
        # features2 = self.backbone(images_norm2.tensor)
        #
        # features = self.post_image_fusion_layer(features1, features2)
        #
        # gt_instances = None
        # mask_feats, _ = self.mask_branch(features, gt_instances)
        #
        # proposals1, _ = self.proposal_generator.forward_test(
        #     images_norm1, features1, gt_instances, self.controller)
        #
        # proposals2, _ = self.proposal_generator.forward_test(
        #     images_norm2, features2, gt_instances, self.controller)
        #
        # padded_im_h, padded_im_w = images_norm1.tensor.size()[-2:]
        # processed_results = []
        # # if not self.using_resize:
    ...
    def forward(self, batched_inputs):
        if self.is_fusion:
            if self.fusion_mode == "pre":
                return self._pre_fusion_forward(batched_inputs)
            elif self.fusion_mode == "post":
                return self._post_fusion_forward(batched_inputs)
            else:
                raise ArgumentTypeError("fusion_mode must be 'pre' or 'post'")
        else:
            return self._sin_image_forward(self.batched_inputs)

    def _sin_image_forward(self, batched_inputs):
        """
        单图片前向传播
        :param batched_inputs:
        :return:
    ...
    def _pre_fusion_forward(self, batched_inputs):
        """
                单图片前向传播
                :param batched_inputs:
                :return:
        ...
        if self.training and self.use_aug:
            original_images1 = [x["image1"].to(self.device) for x in batched_inputs]
            original_images2 = [x["image2"].to(self.device) for x in batched_inputs]
            aug_images1 = [x["aug_image1"].to(self.device) for x in batched_inputs]
            aug_images2 = [x["aug_image2"].to(self.device) for x in batched_inputs]

            # auxiliary augmentation, e.g., random erasing
            images_norm1 = [self.normalizer(x) for x in aug_images1]
            images_norm2 = [self.normalizer(x) for x in aug_images2]
            # images_norm = [self.aux_aug(self.normalizer(x)) for x in aug_images]
        else:
            original_images1 = [x["image1"].to(self.device) for x in batched_inputs]
            original_images2 = [x["image2"].to(self.device) for x in batched_inputs]
        ...
        fusion_image_list = [f_t for f_t in fusion_tensor]
        fusion_image_list = ImageList.from_tensors(
            fusion_image_list, self.backbone.size_divisibility
        )
        ...
        if "instances" in batched_inputs[0]:
            gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
            # add pseudo masks to gt
            self.add_pseudo_bitmasks(gt_instances, fusion_image_list.tensor.size(-2),
                                     fusion_image_list.tensor.size(-1), fusion_image_list.image_sizes)

            original_image_masks = [torch.ones_like(
                x[0], dtype=torch.float32) for x in original_images1]
            ...
            for i in range(len(original_image_masks)):
                ...
                pixels_removed = int(
                    self.bottom_pixels_removed
                    * float(original_images1[i].size(1)) / float(im_h)
                )
                if pixels_removed > 0:
                    original_image_masks[i][-pixels_removed:, :] = 0

            original_images1 = ImageList.from_tensors(
                original_images1, self.backbone.size_divisibility)

            original_images2 = ImageList.from_tensors(
                original_images2, self.backbone.size_divisibility)
            ...
            self.add_bitmasks_from_boxes_with_multi_images(
                gt_instances, original_images1.tensor, original_images2.tensor, original_image_masks.tensor,
                fusion_image_list.tensor.size(-2), fusion_image_list.tensor.size(-1),
            )
            # constraint mask with boxs
            self.constrain_mask_with_box(gt_instances)
            # compute pseudo pairwise affnity
        ...
        if self.training:
            ...
            padded_im_h, padded_im_w = images_norm1.tensor.size()[-2:]
            processed_results = []
            for im_id, (input_per_image, image_size) in enumerate(zip(batched_inputs, images_norm1.image_sizes)):
                height = input_per_image.get("height", image_size[0])
                width = input_per_image.get("width", image_size[1])
    ...
    def _post_fusion_forward(self, batched_inputs):
        pass
    ...
    def _fusion_images_color_similarity(self, images_color_similarity1, images_color_similarity2, mode="avg"):
        if mode == "avg":
            return (images_color_similarity1 + images_color_similarity2) / 2
        elif mode == "max":
            return torch.max(images_color_similarity1, images_color_similarity2)
        elif mode == "min":
            return torch.min(images_color_similarity1, images_color_similarity2)
        elif mode == "sum":
            alpha = 0.5
            beta = 0.5
            return alpha * images_color_similarity1 + beta * images_color_similarity2
        else:
            return images_color_similarity1

    def add_bitmasks_from_boxes_with_multi_images(self, instances, images1, images2, image_masks, im_h, im_w):
        """
        使用多图像添加二值掩码
        """
        stride = self.mask_out_stride
        ...
        assert images1.size(2) % stride == 0 and images2.size(2) % stride == 0
        assert images1.size(3) % stride == 0 and images2.size(3) % stride == 0

        # 下采样图像
        downsampled_images1 = F.avg_pool2d(
            images1.float(), kernel_size=stride,
            stride=stride, padding=0
        )
        downsampled_images2 = F.avg_pool2d(
            images2.float(), kernel_size=stride,
            stride=stride, padding=0
        )
        # BGR 2 RGB
        downsampled_images1 = downsampled_images1[:, [2, 1, 0]]
        downsampled_images2 = downsampled_images2[:, [2, 1, 0]]

        image_masks = image_masks[:, start::stride, start::stride]

        downsampled_images_lab1 = rgb2lab(downsampled_images1.byte().float())
        downsampled_images_lab2 = rgb2lab(downsampled_images2.byte().float())
        for im_i, per_im_gt_inst in enumerate(instances):
            images_lab1 = downsampled_images_lab1[im_i][None]
            images_lab2 = downsampled_images_lab2[im_i][None]
            images_color_similarity1 = get_images_color_similarity(
                images_lab1, image_masks[im_i],
                self.pairwise_size, self.pairwise_dilation,
            )
            images_color_similarity2 = get_images_color_similarity(
                images_lab2, image_masks[im_i],
                self.pairwise_size, self.pairwise_dilation,
            )
            # 平均法得到颜色相似度
            images_color_similarity = self._fusion_images_color_similarity(
                images_color_similarity1, images_color_similarity2, mode="avg"
            )
